...
export const generateRandomPoli = (minimumShares: number, S: number) => {
  const coefficients = []

  for (let i = 1; i < minimumShares; i = i + 1) {
...
export const computeShares = (coefficients: number [], shareDivisions: number, p: number) => {
  const shares = []
...
export const recoverSecret = (shares: Array<bigint>, indexes: number[], p: number) => {
  const coeff: any = []
